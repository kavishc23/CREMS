...
public sealed class DivisionsController(ApplicationDbContext db, CurrentStaffScope staffScope) : ControllerBase
{
    ...
    public async Task<ActionResult<IReadOnlyList<DivisionResponse>>> GetAll(CancellationToken cancellationToken)
    {
        ...
        var query = Query(); if (!scope.IsAdministrator) query = query.Where(x => x.Id == scope.DivisionId);
    ...
    public async Task<ActionResult> UpdateService(Guid divisionId, Guid serviceId, SaveServiceRequest request, CancellationToken cancellationToken)
    {
        ...
        service.IsBookableOnline = request.IsBookableOnline; service.RequiresQuote = request.RequiresQuote; service.DefaultHireUnit = request.DefaultHireUnit; service.RequiresDelivery = request.RequiresDelivery; service.RequiredDocumentsJson = request.RequiredDocumentsJson ?? "[]"; service.DefaultDepositAmount = request.DefaultDepositAmount; service.InspectionRequirementsJson = request.InspectionRequirementsJson ?? "{}"; service.MeterType = Clean(request.MeterType); service.MaintenanceRulesJson = request.MaintenanceRulesJson ?? "{}";
...
public sealed record SaveDivisionRequest([Required, MaxLength(30)] string Code, [Required, MaxLength(150)] string Name,
    ...
    [MaxLength(50)] string? ContactPhone, DivisionCapabilities Capabilities, bool IsPublic, bool IsActive = true, string? BrandingJson = "{}", string DefaultCurrency = "FJD", decimal DefaultTaxRate = 15, string? DefaultRentalTerms = null, Guid? DefaultApprovalWorkflowId = null, string? CustomerBookingConfigurationJson = "{}");
public sealed record SaveServiceRequest([Required, MaxLength(40)] string Code, [Required, MaxLength(150)] string Name,
    ...
    bool IsBookableOnline, bool RequiresQuote, bool IsActive = true, ChargeUnit DefaultHireUnit = ChargeUnit.Day, bool RequiresDelivery = false, string? RequiredDocumentsJson = "[]", decimal DefaultDepositAmount = 0, string? InspectionRequirementsJson = "{}", string? MeterType = null, string? MaintenanceRulesJson = "{}");
public sealed record DivisionResponse(Guid Id, string Code, string Name, string? Description, string? ContactEmail,
    string? ContactPhone, DivisionCapabilities Capabilities, bool IsPublic, bool IsActive, IReadOnlyList<ServiceResponse> Services, string BrandingJson, string DefaultCurrency, decimal DefaultTaxRate, string? DefaultRentalTerms, Guid? DefaultApprovalWorkflowId, string CustomerBookingConfigurationJson);
public sealed record ServiceResponse(Guid Id, string Code, string Name, string? Description, ServiceOfferingType Type,
    PersonnelRequirement PersonnelRequirement, bool IsBookableOnline, bool RequiresQuote, bool IsActive, ChargeUnit DefaultHireUnit, bool RequiresDelivery, string RequiredDocumentsJson, decimal DefaultDepositAmount, string InspectionRequirementsJson, string? MeterType, string MaintenanceRulesJson);
