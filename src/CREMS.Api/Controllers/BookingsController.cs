...
public sealed class BookingsController(ApplicationDbContext db, CurrentStaffScope staffScope, IAuthorizationService authorization) : ControllerBase
{
    ...
    public async Task<ActionResult> GetWorkspace(Guid id, CancellationToken cancellationToken)
    {
        ...
        return Ok(new {
            ...
            item = item is null ? null : new { item.Id, item.AssetId, item.Asset!.AssetNumber, item.Asset.Name, item.Asset.Category, assetStatus = item.Asset.Status.ToString(), item.Asset.DivisionId, division = item.Asset.Division?.Name, item.Asset.ServiceOfferingId, service = item.Asset.ServiceOffering?.Name, item.StartAt, item.EndAt, item.DailyRate, item.Asset.CurrentMeterReading, item.Asset.MeterUnit },
