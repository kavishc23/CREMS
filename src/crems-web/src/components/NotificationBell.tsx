...
export function NotificationBell({customer=false,canSend=false}:{customer?:boolean;canSend?:boolean}) {
  ...
  const load=useCallback(async(signal?:AbortSignal)=>{
    try {
      ...
      const fresh=latest.data.items.filter(n=>!n.isRead&&!n.isExpired&&n.toastEnabled&&!seen.current?.has(n.id))
      ...
      latest.data.items.forEach(n=>seen.current?.add(n.id))
      setCount(latest.data.unreadCount)
