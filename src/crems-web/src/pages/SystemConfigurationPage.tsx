...
export function SystemConfigurationPage() {
  ...
  async function load() {
    try {
      ...
      setSettings(settingResponse.data); setTemplates(templateResponse.data); setHealth(healthResponse.data); setAudit(auditResponse.data); setError('')
  ...
  return <Box><Stack direction="row" gap={1.5} alignItems="center" sx={{ px: { xs: 2, sm: 3, lg: 4 }, pt: 3, pb: 2 }}><SettingsOutlined /><Box><Typography variant="h4" fontWeight={800}>System configuration</Typography><Typography color="text.secondary">Manage operating defaults, rates, communications and technical controls.</Typography></Box></Stack>
    ...
    <Tabs value={tab} onChange={(_, value) => setTab(value)} variant="scrollable" sx={{ px: { xs: 2, sm: 3, lg: 4 }, borderBottom: 1, borderColor: 'divider' }}><Tab label="Overview" /><Tab label="Rental defaults" /><Tab label="Rates & charges" /><Tab label="Notifications" /><Tab label="System health & audit" /></Tabs>
    {tab === 0 && <Box sx={{ p: { xs: 2, sm: 3, lg: 4 } }}><Grid container spacing={2}>{[
      ['Rental defaults', 'Tax, currency, deposits and booking hold rules.'], ['Rates & charges', 'Reusable customer rates and internal cost definitions.'], ['Notifications', 'Customer email wording and delivery configuration.'], ['System health', 'Database, messaging, version and administrator activity.'],
