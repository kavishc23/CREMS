...
export type RentalAgreementData = {
  ...
  terms: AgreementTerm[]; customerSignatureName: string | null; customerSignedAt: string | null; approvedByName: string | null; approvedAt: string | null
}
...
export function createRentalAgreementPdf(data: RentalAgreementData, logo: string) {
  ...
  sectionTitle('Signatures and approval'); ensure(48); doc.setTextColor(20); doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.text('By signing below, the customer accepts this agreement and acknowledges receiving a copy.', margin + 2, y); y += 11; doc.setDrawColor(80); doc.line(margin + 2, y + 11, margin + 80, y + 11); doc.line(margin + 98, y + 11, pageWidth - margin - 2, y + 11); doc.setFont('helvetica', 'bold'); doc.text(data.customerSignatureName || '', margin + 2, y + 7); doc.text(data.approvedByName || '', margin + 98, y + 7); doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5); doc.setTextColor(90); doc.text(`Customer signature | ${data.customerSignedAt ? new Date(data.customerSignedAt).toLocaleString('en-FJ') : ''}`, margin + 2, y + 16); doc.text(`Approved by rental agent | ${data.approvedAt ? new Date(data.approvedAt).toLocaleString('en-FJ') : ''}`, margin + 98, y + 16)
