...
function PageHeading({title,subtitle,back}:{title:string;subtitle:string;back:()=>void}){return <Stack direction={{xs:'column',sm:'row'}} justifyContent="space-between" gap={2} mb={3}><Box><Typography variant="h4" fontWeight={850}>{title}</Typography><Typography color="text.secondary">{subtitle}</Typography></Box><Button onClick={back}>Back to operations</Button></Stack>}
