...
export function PublicRentalPage({ onCustomerAccount, onCustomerSignOut, customerAuthenticated = false, customerName }: { onCustomerAccount: (section?: 'overview' | 'bookings' | 'quotes' | 'documents' | 'receipts' | 'account') => void; onCustomerSignOut?: () => void | Promise<void>; customerAuthenticated?: boolean; customerName?: string }) {
  ...
  const professionalDriverSelected = Boolean((checkoutDetail?.charges ?? []).find(charge => /driver/i.test(`${charge.name} ${charge.category}`) && (selectedExtras[charge.id] ?? 0) > 0))
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: '#f7f7f3' }}>
    ...
    <Container id="rentals" maxWidth={false} sx={{ py: { xs: 5, md: 6 }, px: { xs: 2, md: 5, lg: 7 } }}>
      ...
      {searched && <Card variant="outlined" sx={{ mb: 3, bgcolor: 'white' }}><CardContent sx={{ p: 2 }}><Stack direction={{ xs: 'column', md: 'row' }} spacing={1.25} alignItems={{ md: 'center' }}>
    ...
    <Dialog open={Boolean(selected)} onClose={() => !submitting && setCancelConfirmOpen(true)} fullWidth maxWidth="xl" slotProps={{ paper: { sx: { height: 'min(820px, calc(100vh - 110px))', m: { xs: 1.5, md: 3 }, overflow: 'hidden' } } }}><DialogTitle sx={{ pr: 7, position: 'relative', py: 1.5 }}>{reference ? 'Request received' : quotationFlow ? 'Request a quotation' : 'Book your vehicle'}{!reference && <IconButton aria-label="Cancel booking" onClick={() => setCancelConfirmOpen(true)} sx={{ position: 'absolute', top: 6, right: 12 }}><CloseOutlined /></IconButton>}</DialogTitle><DialogContent dividers sx={{ p: { xs: 2, md: 3 }, overflow: 'hidden' }}>
      ...
      <Box component="form" id="public-booking-form" onSubmit={bookingStep === 3 ? submitRequest : (event) => { event.preventDefault(); setBookingStep(step => step + 1) }}>
        <Stepper activeStep={bookingStep} alternativeLabel sx={{ py: 1.25 }}>{['Dates & branch', 'Rental & extras', 'Your details', 'Price & submit'].map(label => <Step key={label}><StepLabel>{label}</StepLabel></Step>)}</Stepper>
    ...
    <Dialog open={cancelConfirmOpen} onClose={() => setCancelConfirmOpen(false)} maxWidth="xs" fullWidth><DialogTitle>Cancel this booking?</DialogTitle><DialogContent><Typography color="text.secondary">Your entered booking details will be deleted and cannot be restored.</Typography></DialogContent><DialogActions><Button onClick={() => setCancelConfirmOpen(false)}>Keep booking</Button><Button color="error" variant="contained" onClick={cancelBooking}>Cancel booking</Button></DialogActions></Dialog>
  </Box>
}
