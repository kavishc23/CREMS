...
import {
  ...
  InputLabel, MenuItem, Pagination, Select, Stack, Tab, Tabs, Table, TableBody, TableCell,
...
type Asset = {
  ...
  inheritBond: boolean; personnelOverride: string | null; currentLocation: string | null
  photoUrlsJson: string; serviceOfferingId: string | null; assetCategoryId: string | null; personnelRequirement: 'None'|'Optional'|'Required'; requiresDelivery:boolean
}
type Branch = { id: string; name: string; isActive: boolean; divisionIds: string[] }
type Division = { id: string; name: string; isActive: boolean; services: { id: string; name: string; isActive: boolean }[] }
...
type AssetForm = {
  inheritBond: boolean; personnelOverride: string; currentLocation: string
  ...
  acquisitionDate: string; acquisitionCost: string; currentBookValue: string; ownershipType: string; insurancePolicyNumber: string; insuranceExpiry: string; warrantyExpiry: string; photoUrlsJson: string; serviceOfferingId:string; assetCategoryId:string; personnelRequirement:'None'|'Optional'|'Required'; requiresDelivery:boolean
}
const emptyForm: AssetForm = {
  inheritBond: true, personnelOverride: '', currentLocation: '',
  ...
  category: '', manufacturer: '', model: '', modelYear: '', vinOrChassisNumber: '', engineNumber: '', meterUnit: 'km', currentMeterReading: '', acquisitionDate: '', acquisitionCost: '', currentBookValue: '', ownershipType: 'Owned', insurancePolicyNumber: '', insuranceExpiry: '', warrantyExpiry: '', photoUrlsJson: '[]', serviceOfferingId:'', assetCategoryId:'', personnelRequirement:'None', requiresDelivery:false,
}
...
export function AssetsPage({ userRoles }: { userRoles: string[] }) {
  ...
  const [customerPreviewAsset, setCustomerPreviewAsset] = useState<Asset | null>(null)
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      const assetResponse = await api.get<Asset[]>('/assets', { params: { search: debouncedSearch || undefined, divisionId:divisionFilter||undefined, branchId:branchFilter||undefined, category:categoryFilter||undefined, status:statusFilter||undefined, page, pageSize: assetPageSize } })
  ...
  function openCreate() {
    const divisionId = divisions[0]?.id ?? ''; setEditing(null); setForm({ ...emptyForm, divisionId, branchId: branches.find(x => x.divisionIds.includes(divisionId))?.id ?? '' }); setEditTab(0); setError(''); setOpen(true)
  }

  function openEdit(asset: Asset) {
    setEditing(asset)
    setForm({ inheritBond: asset.inheritBond ?? false, personnelOverride: asset.personnelOverride ?? '', currentLocation: asset.currentLocation ?? '', assetNumber: asset.assetNumber, name: asset.name, type: asset.type, status: asset.status,
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      setError(errors ? Object.values(errors).flat().join(' ') : data?.message ?? data?.detail ?? 'Unable to save the asset.')
  ...
  return <Box sx={{
    ...
    ].map(([label,value,color])=><Card key={String(label)} variant="outlined"><CardContent sx={{p:1.5,'&:last-child':{pb:1.5}}}><Typography variant="h5" fontWeight={850} color={String(color)}>{value}</Typography><Typography variant="caption" color="text.secondary" fontWeight={750}>{label}</Typography></CardContent></Card>)}</Box>
    <Box sx={{display:'grid',gridTemplateColumns:{xs:'1fr',lg:'minmax(0,1fr) minmax(310px,390px)'},gap:1.5,alignItems:'start'}}><Card variant="outlined" sx={{overflow:'hidden'}}><CardContent sx={{p:0}}>
      <Stack direction={{xs:'column',md:'row'}} gap={1} sx={{ p: 1.25, borderBottom: 1, borderColor: 'divider', flexShrink:0 }}><TextField size="small" placeholder="Search asset, registration, VIN or serial" value={search}
      ...
      {loading ? <Box sx={{ minHeight: 280, display: 'grid', placeItems: 'center' }}><CircularProgress /></Box> :
        <TableContainer><Table size="small"><TableHead><TableRow sx={{ bgcolor: '#f6f6f3' }}>
      ...
      <Stack direction={{xs:'column',sm:'row'}} justifyContent="space-between" alignItems={{xs:'stretch',sm:'center'}} gap={1.25} sx={{p:1.5,borderTop:1,borderColor:'divider'}}><Typography variant="body2" color="text.secondary">Showing {totalAssets===0?0:(page-1)*assetPageSize+1}–{Math.min(page*assetPageSize,totalAssets)} of {totalAssets.toLocaleString()}</Typography><Stack direction="row" alignItems="center" justifyContent="flex-end" gap={0.75}><Button size="small" variant="outlined" disabled={page<=1} onClick={()=>setPage(current=>Math.max(1,current-1))}>Previous</Button><Pagination page={page} count={Math.max(1,Math.ceil(totalAssets/assetPageSize))} onChange={(_,value)=>setPage(value)} color="primary" siblingCount={0} boundaryCount={1} hidePrevButton hideNextButton/><Button size="small" variant="outlined" disabled={page>=Math.ceil(totalAssets/assetPageSize)} onClick={()=>setPage(current=>Math.min(Math.ceil(totalAssets/assetPageSize),current+1))}>Next</Button></Stack></Stack>
    </CardContent></Card><Card variant="outlined" sx={{position:{lg:'sticky'},top:{lg:16},height:{lg:'calc(100dvh - 32px)'},overflow:'hidden',display:{xs:selectedAsset?'block':'none',lg:'block'}}}>{selectedAsset?<><Box sx={{height:{xs:180,lg:'clamp(150px,26vh,240px)'},bgcolor:'grey.100',position:'relative',flexShrink:0}}>{photoUrls(selectedAsset)[0]&&<Box component="img" src={photoUrls(selectedAsset)[0]} alt={selectedAsset.name} sx={{width:'100%',height:'100%',objectFit:'contain'}}/>}<IconButton aria-label="QR label" onClick={()=>setQrAsset(selectedAsset)} sx={{position:'absolute',right:12,bottom:12,bgcolor:'white','&:hover':{bgcolor:'grey.100'}}}><QrCode2Outlined/></IconButton></Box><CardContent sx={{p:{xs:2,lg:2.25},height:{lg:'calc(100% - clamp(150px,26vh,240px))'},display:'flex',flexDirection:'column','&:last-child':{pb:{xs:2,lg:2.25}}}}><Typography variant="overline" color="text.secondary">{selectedAsset.assetNumber}</Typography><Typography variant="h5" fontWeight={850} lineHeight={1.15}>{selectedAsset.name}</Typography><Typography variant="body2" color="text.secondary" mt={0.5}>{selectedAsset.registrationNumber||selectedAsset.serialNumber||'Identification pending'} · {selectedAsset.branchName}</Typography><Stack direction="row" gap={1} mt={1.5} flexWrap="wrap"><Button variant="contained" size="small" onClick={()=>setProfileAssetId(selectedAsset.id)}>Open full profile</Button>{canManage&&<Button variant="outlined" size="small" startIcon={<EditOutlined/>} onClick={()=>openEdit(selectedAsset)}>Edit</Button>}</Stack><Chip size="small" label={selectedAsset.status.replace(/([a-z])([A-Z])/g,'$1 $2')} color={statusColors[selectedAsset.status]??'default'} sx={{alignSelf:'flex-start',mt:1.25}}/><Box sx={{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',columnGap:2,rowGap:{xs:1.25,lg:'clamp(8px,1.4vh,16px)'},mt:1.5,pt:1.5,borderTop:1,borderColor:'divider'}}>{[['Division',selectedAsset.divisionName||'—'],['Category',selectedAsset.category||selectedAsset.type],['Meter',selectedAsset.currentMeterReading==null?'Not recorded':`${selectedAsset.currentMeterReading.toLocaleString()} ${selectedAsset.meterUnit||''}`],['Rate',`FJD ${selectedAsset.dailyRate.toFixed(2)}/day`],['Refundable bond',selectedAsset.inheritBond?'Service / division default':`FJD ${(selectedAsset.defaultBondAmount??0).toFixed(2)}`],['Next service',selectedAsset.nextServiceDate?new Date(selectedAsset.nextServiceDate).toLocaleDateString('en-FJ'):'Not scheduled'],['Model',[selectedAsset.manufacturer,selectedAsset.model,selectedAsset.modelYear].filter(Boolean).join(' ')||'—']].map(([label,value],index)=><Box key={label} sx={index===6?{gridColumn:'1 / -1'}:undefined}><Typography variant="caption" color="text.secondary" display="block">{label}</Typography><Typography variant="body2" fontWeight={750} lineHeight={1.25}>{value}</Typography></Box>)}</Box>{canManagePhotos&&<Stack direction="row" gap={0.5} mt="auto" pt={1.25} flexWrap="wrap" sx={{borderTop:1,borderColor:'divider'}}><Button size="small" startIcon={<ImageOutlined/>} onClick={()=>setPhotoAsset(selectedAsset)}>Photos</Button>{canManage&&<><Button size="small" startIcon={<InsightsOutlined/>} onClick={()=>void openPerformance(selectedAsset)}>Financials</Button><Button size="small" startIcon={<QrCode2Outlined/>} onClick={()=>setQrAsset(selectedAsset)}>QR label</Button></>}</Stack>}</CardContent></>:<CardContent><Typography color="text.secondary">Choose an asset to see its details.</Typography></CardContent>}</Card></Box>
    <Dialog open={open} onClose={() => !saving && setOpen(false)} fullWidth maxWidth="lg" slotProps={{ paper: { sx: { maxHeight: 'calc(100dvh - 32px)', height: { xs: '100dvh', sm: 'auto' } } } }}>
      <Box component="form" onSubmit={save}><DialogTitle sx={{ pb: 1 }}>{editing ? 'Edit asset' : 'Add asset'}<Typography variant="body2" color="text.secondary" mt={.5}>Customer listing, operational controls, and internal records are grouped below.</Typography></DialogTitle><DialogContent dividers sx={{ overflowY: 'auto', '& .editor-row': { display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 2, alignItems: 'start' }, '& .editor-row > *': { minWidth: 0 }, '@media (max-width: 900px)': { '& .editor-row': { gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' } }, '@media (max-width: 560px)': { px: 2, '& .editor-row': { gridTemplateColumns: '1fr', gap: 1.5 } } }}>
        <Stack spacing={2.25} mt={1}>{error && <Alert severity="error">{error}</Alert>}
          <Stack className="editor-row">
          ...
          <Stack className="editor-row">
            ...
            <TextField select fullWidth label="Service offering" value={form.serviceOfferingId} onChange={e => setForm({ ...form, serviceOfferingId: e.target.value, assetCategoryId: '', category: '' })}><MenuItem value="">No service</MenuItem>{divisions.find(x => x.id === form.divisionId)?.services.filter(x => x.isActive).map(x => <MenuItem key={x.id} value={x.id}>{x.name}</MenuItem>)}</TextField>
            <FormControl fullWidth><InputLabel>Asset category</InputLabel><Select label="Asset category" value={form.assetCategoryId} onChange={(e)=>{const selected=assetCategories.find(x=>x.id===e.target.value);setForm({...form,assetCategoryId:e.target.value,serviceOfferingId:selected?.serviceOfferingId??form.serviceOfferingId,category:selected?.name??'',meterUnit:selected?.defaultMeterType??form.meterUnit,requiresDelivery:selected?.code!=='RENTAL_VEHICLE',type:selected?.code==='RENTAL_VEHICLE'?'PassengerVehicle':selected?.code==='HEAVY_MACHINE'?'HeavyEquipment':selected?.code==='FORKLIFT'?'MaterialHandlingEquipment':selected?.code==='GENSET'?'PowerEquipment':selected?.code==='SCAFFOLD'?'Scaffolding':selected?.code==='PORTABLE_TOILET'?'PortableSanitation':selected?.code==='BIG_BIN'?'WasteContainer':'LightEquipment'})}}>{assetCategories.filter(x=>x.divisionId===form.divisionId&&(!form.serviceOfferingId||!x.serviceOfferingId||x.serviceOfferingId===form.serviceOfferingId)).map(value=><MenuItem key={value.id} value={value.id}>{value.name} ({value.personnelRequirement==='Required'?'operator required':value.personnelRequirement==='Optional'?'operator optional':'no operator'})</MenuItem>)}</Select></FormControl>
            <FormControl fullWidth required><InputLabel>Customer fulfilment</InputLabel><Select label="Customer fulfilment" value={form.requiresDelivery?'DeliveryOnly':'PickupOrDelivery'} onChange={e=>setForm({...form,requiresDelivery:e.target.value==='DeliveryOnly'})}><MenuItem value="PickupOrDelivery">Pickup or delivery</MenuItem><MenuItem value="DeliveryOnly">Delivery only</MenuItem></Select></FormControl>
          ...
          <Stack className="editor-row">
            ...
            <TextField select fullWidth label="Refundable bond policy" value={form.inheritBond ? 'inherit' : 'fixed'} onChange={e => setForm({ ...form, inheritBond: e.target.value === 'inherit' })}><MenuItem value="inherit">Use service / division default</MenuItem><MenuItem value="fixed">Asset-specific amount (0 = not required)</MenuItem></TextField>
            <TextField select fullWidth label="Professional driver / operator" helperText="Inherit follows category, then service. None disables personnel charges for this asset." value={form.personnelOverride} onChange={e => setForm({ ...form, personnelOverride: e.target.value })}><MenuItem value="">Use category / service policy</MenuItem>{['None','Optional','Required'].map(value => <MenuItem key={value} value={value}>{value}</MenuItem>)}</TextField>
            <TextField fullWidth required disabled={form.inheritBond} type="number" label="Refundable bond (FJD)" helperText="Separate from hire and VAT. Applies to new requests only." inputProps={{ min: 0, step: '0.01' }} value={form.defaultBondAmount} onChange={(e) => setForm({ ...form, defaultBondAmount: e.target.value })} />
    ...
    <AssetProfileDialog assetId={profileAssetId} onClose={() => setProfileAssetId(null)} onEdit={assetId => { const asset = assets.find(item => item.id === assetId); if (asset) { setProfileAssetId(null); openEdit(asset) } }} onPreview={assetId => { const asset = assets.find(item => item.id === assetId); if (asset) setCustomerPreviewAsset(asset) }} />
    <PublicAssetDetailDialog asset={customerPreviewAsset ? { id: customerPreviewAsset.id, name: customerPreviewAsset.name, type: customerPreviewAsset.type, branchName: customerPreviewAsset.branchName, dailyRate: customerPreviewAsset.dailyRate, isAvailable: customerPreviewAsset.status === 'Available' } : null} imageUrls={customerPreviewAsset ? photoUrls(customerPreviewAsset) : []} startDate={new Date(Date.now() + 86400000).toISOString().slice(0, 10)} endDate={new Date(Date.now() + 3 * 86400000).toISOString().slice(0, 10)} open={Boolean(customerPreviewAsset)} onClose={() => setCustomerPreviewAsset(null)} onContinue={() => setCustomerPreviewAsset(null)} previewDetails={customerPreviewAsset ? { id: customerPreviewAsset.id, name: customerPreviewAsset.name, type: customerPreviewAsset.type, category: customerPreviewAsset.category, dailyRate: customerPreviewAsset.dailyRate, personnelRequirement: customerPreviewAsset.personnelRequirement, manufacturer: customerPreviewAsset.manufacturer, model: customerPreviewAsset.model, modelYear: customerPreviewAsset.modelYear, division: customerPreviewAsset.divisionId && customerPreviewAsset.divisionName ? { id: customerPreviewAsset.divisionId, code: '', name: customerPreviewAsset.divisionName } : null, branch: { id: customerPreviewAsset.branchId, name: customerPreviewAsset.branchName, address: null, phone: null, email: null, pickupInstructions: null, deliveryCoverage: null }, service: null, specifications: [], charges: [], isAvailable: customerPreviewAsset.status === 'Available', availabilityChecked: true } : undefined} />
