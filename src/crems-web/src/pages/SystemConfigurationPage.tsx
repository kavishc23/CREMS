...
export function SystemConfigurationPage() {
  ...
  async function load() {
    try {
      ...
      setSettings(settingResponse.data.filter(item => item.key !== 'rentals.defaultDeposit')); setTemplates(templateResponse.data); setHealth(healthResponse.data); setAudit(auditResponse.data); setError('')
  ...
  const overviewSections = [
    { title: 'Rental defaults', detail: 'Tax, currency, refundable bonds and booking hold rules.', tab: 1 },
    { title: 'Rates & charges', detail: 'Reusable customer rates and internal cost definitions.', tab: 2 },
    ...(isNotificationModuleEnabled ? [{ title: 'Notifications', detail: 'Customer email wording and delivery configuration.', tab: 3 }] : []),
    { title: 'System health', detail: 'Database, version and administrator activity.', tab: 4 },
    { title: 'Approval rules', detail: 'Route higher-risk bookings through sequential approval stages.', tab: 5 },
  ]

  return <Box><Box sx={{ px: { xs: 2, sm: 3, lg: 4 }, pt: 3, pb: 2 }}><PageHeader icon={<SettingsOutlined />} title="System configuration" subtitle="Manage operating defaults, rates and technical controls." /></Box>
    {error && <Alert severity="error" sx={{ mx: { xs: 2, sm: 3, lg: 4 }, mb: 2 }}>{error}</Alert>}
    <Tabs value={tab} onChange={(_, value) => setTab(value)} variant="scrollable" sx={{ px: { xs: 2, sm: 3, lg: 4 }, borderBottom: 1, borderColor: 'divider' }}><Tab value={0} label="Overview" /><Tab value={1} label="Rental defaults" /><Tab value={2} label="Rates & charges" />{isNotificationModuleEnabled && <Tab value={3} label="Notifications" />}<Tab value={4} label="System health & audit" /><Tab value={5} label="Approval rules" /></Tabs>
    {tab === 0 && <Box sx={{ p: { xs: 2, sm: 3, lg: 4 } }}><Grid container spacing={2}>{overviewSections.map(({ title, detail, tab: targetTab }) => <Grid key={title} size={{ xs: 12, md: 6 }}><Card variant="outlined" sx={{ height: '100%', cursor: 'pointer' }} onClick={() => setTab(targetTab)}><CardContent><Typography variant="h6" fontWeight={800}>{title}</Typography><Typography color="text.secondary" mt={.5}>{detail}</Typography><Button sx={{ mt: 2 }}>Open</Button></CardContent></Card></Grid>)}</Grid></Box>}
