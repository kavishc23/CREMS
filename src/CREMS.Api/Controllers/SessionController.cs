    ...
    IEmailQueue emailQueue) : ControllerBase
{
    ...
    private async Task CompleteSignIn(ApplicationUser user)
    {
        ...
        var windowId = Request.Headers["X-CREMS-Window-Id"].ToString(); db.UserSessions.Add(new UserSession { UserId = user.Id, WindowId = windowId, IpAddress = user.LastLoginIp, UserAgent = Request.Headers.UserAgent.ToString(), DeviceLabel = Device(Request.Headers.UserAgent.ToString()), ExpiresAt = now.AddHours(8) });
