      ...
      <Box component="form" onSubmit={save}><DialogTitle>{editing ? 'Edit asset' : 'Add asset'}</DialogTitle><DialogContent>
        <Stack spacing={2.25} mt={1}>{error && <Alert severity="error">{error}</Alert>}
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
            ...
            <FormControl fullWidth required><InputLabel>Asset category</InputLabel><Select label="Asset category" value={form.assetCategoryId} onChange={(e)=>{const selected=assetCategories.find(x=>x.id===e.target.value);setForm({...form,assetCategoryId:e.target.value,serviceOfferingId:selected?.serviceOfferingId??'',category:selected?.name??'',meterUnit:selected?.defaultMeterType??form.meterUnit,personnelRequirement:selected?.personnelRequirement??'None',requiresDelivery:selected?.code!=='RENTAL_VEHICLE',type:selected?.code==='RENTAL_VEHICLE'?'PassengerVehicle':selected?.code==='HEAVY_MACHINE'?'HeavyEquipment':selected?.code==='FORKLIFT'?'MaterialHandlingEquipment':selected?.code==='GENSET'?'PowerEquipment':selected?.code==='SCAFFOLD'?'Scaffolding':selected?.code==='PORTABLE_TOILET'?'PortableSanitation':selected?.code==='BIG_BIN'?'WasteContainer':'LightEquipment'})}}>{assetCategories.filter(x=>x.divisionId===form.divisionId).map(value=><MenuItem key={value.id} value={value.id}>{value.name}</MenuItem>)}</Select></FormControl>
