...
type Context = { templates:Template[]; preHire:null|{conditionNotes:string|null;damageNotes:string|null;evidenceJson:string;meterReading:number|null;fuelLevelPercent:number|null} }
export function useRentalInspection(bookingId:string|undefined, stage:'PreHire'|'PostHire'){
  ...
  return {items,templateName:template?.name,preHire:current?.data?.preHire,error:current?.error||templateError,loading:Boolean(bookingId&&!current)}
}
