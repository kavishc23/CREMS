  ...
  return <Box sx={{ minHeight: '100dvh', bgcolor: 'background.default', overflowX: 'hidden' }}>
    ...
    {!session ? <Container maxWidth="sm" sx={{ py: 6 }}><Card variant="outlined"><CardContent sx={{ p: { xs: 3, sm: 4 } }}>
      ...
      <Paper square elevation={0} sx={{ minHeight: 'calc(100dvh - 76px)', border: 0, bgcolor: 'background.default' }}>
        <Box sx={{ py: { xs: 4, md: 6 }, px: { xs: 2, md: 5, lg: 7 }, minWidth: 0, maxWidth: 1536, mx: 'auto' }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ sm: 'center' }} justifyContent="space-between" gap={2} mb={4}><Box><Typography variant="h4" fontWeight={800}>{accountSections.find(item => item.value === section)?.label}</Typography><Typography color="text.secondary">{session.customerName} · {session.customerNumber}</Typography></Box></Stack>
...
function RentalJourneys({ bookings, quotes, search, filter, newestFirst, page, expanded, loading, onSearch, onFilter, onSort, onPage, onExpand, onOpenBooking, onChangeBooking, onViewQuote, onBrowse }: {
  bookings: Booking[]; quotes: Quote[]; search: string; filter: ActivityFilter; newestFirst: boolean; page: number; expanded: string | null; loading: boolean
  onSearch: (value: string) => void; onFilter: (value: ActivityFilter) => void; onSort: (value: boolean) => void; onPage: (value: number) => void; onExpand: (value: string | null) => void
  onOpenBooking: (id: string) => void; onChangeBooking: (booking: Booking, type: 'dates' | 'cancel' | 'extend' | 'incident') => void; onViewQuote: (quote: Quote) => void; onBrowse: () => void
}) {
  ...
  return <Stack spacing={2.5}>
    ...
    <Card variant="outlined"><CardContent sx={{ p: 1.5 }}><Grid container spacing={1.25}><Grid size={{ xs: 12, md: 8 }}><TextField fullWidth size="small" placeholder="Search reference, rental or supplier" value={search} onChange={event => onSearch(event.target.value)} InputProps={{ startAdornment: <InputAdornment position="start"><SearchOutlined /></InputAdornment> }} /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField select fullWidth size="small" label="Sort" value={newestFirst ? 'action-first' : 'oldest'} onChange={event => onSort(event.target.value === 'action-first')}><MenuItem value="action-first">Action first</MenuItem><MenuItem value="oldest">Oldest first</MenuItem></TextField></Grid></Grid><Stack direction="row" gap={.75} flexWrap="wrap" mt={1.25}>{filters.map(item => <Button key={item.value} size="small" variant={filter === item.value ? 'contained' : 'outlined'} color={filter === item.value ? 'primary' : 'inherit'} onClick={() => onFilter(item.value)} sx={{ borderRadius: 2 }}>{item.label}{item.value === 'needs-action' && attention.length > 0 ? ` · ${attention.length}` : ''}</Button>)}<Button size="small" color="inherit" onClick={() => { onSearch(''); onFilter('all') }}>Clear filters</Button></Stack></CardContent></Card>
    {attention.length > 0 && <Box><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'baseline' }} gap={.5} mb={1.25}><Typography variant="h6" fontWeight={800}>Needs your attention</Typography><Typography variant="caption" color="text.secondary">Requests and quotations requiring a decision.</Typography></Stack><Grid container spacing={1.25}>{attention.slice(0, 2).map(journey => <Grid key={journey.id} size={{ xs: 12, md: 6 }}><Card variant="outlined"><JourneyCard journey={journey} compact loading={loading} onExpand={onExpand} expanded={expanded === journey.id} onOpenBooking={onOpenBooking} onChangeBooking={onChangeBooking} onViewQuote={onViewQuote} /></Card></Grid>)}</Grid></Box>}
    <Box><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'baseline' }} gap={.5} mb={1.25}><Typography variant="h6" fontWeight={800}>{filter === 'quotations' ? 'Quotations' : filter === 'bookings' ? 'Bookings' : 'All rental journeys'}</Typography><Typography variant="caption" color="text.secondary">{filtered.length} result{filtered.length === 1 ? '' : 's'} · showing {filtered.length ? currentPage * pageSize + 1 : 0}–{Math.min((currentPage + 1) * pageSize, filtered.length)}</Typography></Stack>{pageJourneys.length ? <Stack spacing={1.25}>{pageJourneys.map(journey => <Card key={journey.id} variant="outlined"><JourneyCard journey={journey} loading={loading} onExpand={onExpand} expanded={expanded === journey.id} onOpenBooking={onOpenBooking} onChangeBooking={onChangeBooking} onViewQuote={onViewQuote} /></Card>)}</Stack> : <EmptyState title="No matching rental journeys" text="Try another filter or browse rentals to start a new request." action="Browse rentals" onAction={onBrowse} />}</Box>
...
function JourneyCard({ journey, compact = false, expanded, loading, onExpand, onOpenBooking, onChangeBooking, onViewQuote }: { journey: Journey; compact?: boolean; expanded: boolean; loading: boolean; onExpand: (id: string | null) => void; onOpenBooking: (id: string) => void; onChangeBooking: (booking: Booking, type: 'dates' | 'cancel' | 'extend' | 'incident') => void; onViewQuote: (quote: Quote) => void }) {
  const booking = journey.booking
  return <Box sx={{ p: 2, borderLeft: 4, borderLeftColor: statusAccent(journey.status), borderBottom: compact ? 0 : 1, borderBottomColor: 'divider', '&:last-child': { borderBottom: 0 } }}><Grid container spacing={1.5} alignItems="center"><Grid size={{ xs: 12, md: compact ? 12 : 7 }}><Typography variant="overline">Rental journey · {journey.kind === 'booking' ? journey.status === 'ConvertedToRental' ? 'On hire' : journey.status === 'Draft' ? 'Request' : 'Booking' : 'Quotation'}</Typography><Typography fontWeight={800}>{journey.title}</Typography><Typography variant="body2" color="text.secondary">{journey.meta}</Typography><Typography variant="caption" color="text.secondary" display="block" mt={.5}>{journey.reference}</Typography><Stack direction="row" gap={.75} flexWrap="wrap" mt={1}>{booking ? <><Button size="small" variant="contained" disabled={loading} onClick={() => onOpenBooking(booking.id)}>{booking.status === 'Draft' ? 'Manage request' : 'Manage rental'}</Button>{booking.status === 'Draft' && <Button size="small" variant="outlined" onClick={() => onChangeBooking(booking, 'dates')}>Change dates</Button>}{['Confirmed', 'ConvertedToRental'].includes(booking.status) && <Button size="small" variant="outlined" onClick={() => onChangeBooking(booking, 'extend')}>Request extension</Button>}</> : <Button size="small" variant="contained" onClick={() => journey.quote && onViewQuote(journey.quote)}>{journey.attention ? 'Review quotation' : 'View quotation'}</Button>}</Stack>{!compact && <Button size="small" color="inherit" sx={{ mt: .75, px: 0 }} onClick={() => onExpand(expanded ? null : journey.id)}>{expanded ? '▴ Hide journey history' : '▾ View journey history'}</Button>}{expanded && <Box sx={{ mt: 1, p: 1.25, borderRadius: 1.5, bgcolor: 'action.hover' }}><Typography variant="caption" color="text.secondary">{booking ? `Booking ${booking.reference} is ${statusLabel(booking.status).toLowerCase()}. Rental details, documents and support history are available through Manage rental.` : `Quotation ${journey.quote?.quoteNumber} is ${statusLabel(journey.status).toLowerCase()}. Open it to review pricing and respond.`}</Typography></Box>}</Grid><Grid size={{ xs: 6, md: compact ? 6 : 2 }}><Typography variant="body2">{journey.dateText}</Typography></Grid><Grid size={{ xs: 6, md: compact ? 6 : 3 }}><Stack alignItems={{ xs: 'flex-start', md: 'flex-end' }} gap={.75}><Chip size="small" label={statusLabel(journey.status)} color={statusColor(journey.status)} />{journey.total != null && <Typography fontWeight={850}>{money(journey.total)}</Typography>}</Stack></Grid></Grid></Box>
}
